import tkinter
import tkinter.messagebox
import tkinter.ttk
import pygetwindow as gw  # Install dependency with `pip install pygetwindow`
from random import randint


# Defining Window Properties
root = tkinter.Tk()
root.withdraw()


def getPos(event):
    """Get the position of the window"""

    xwin = window.winfo_x()
    ywin = window.winfo_y()
    startx = event.x_root
    starty = event.y_root

    ywin = ywin - starty
    xwin = xwin - startx

    def moveWindow(event):
        """Moving the window on mouse move"""

        window.geometry(
            "1000x310" + f'+{event.x_root + xwin}+{event.y_root + ywin}')

    startx = event.x_root
    starty = event.y_root

    window.bind('<B1-Motion>', moveWindow)


def topOrNot():
    """
    Detects wheather the window should be shown or not.

    Makes it act like a Desktop widget.
    """

    # TODO: Refine this logic
    # HELP WANTED

    windows = gw.getActiveWindow()

    # Desktop Widget logic
    if windows is None:
        window.deiconify()
        window.lift()
        window.attributes("-topmost", True)
    else:
        if windows.isMaximized:
            window.lower()
            window.attributes("-topmost", False)
        elif (not windows.isMaximized and windows.title != "" and
                windows.title != "Widget"):
            window.deiconify()
            window.attributes("-topmost", False)
            window.lower()
        elif (not windows.isMaximized and windows.title != "" and
                windows.title == "Widget"):
            window.attributes("-topmost", False)
        elif windows.title == "tk":
            window.deiconify()
            window.lift()
            window.attributes("-topmost", True)
        else:
            window.deiconify()
            window.lift()
            window.attributes("-topmost", True)

    window.after(10, topOrNot)


window = tkinter.Toplevel()
window.title("Widget")
window.attributes("-toolwindow", True)
window.overrideredirect(1)
window.config(bg="white")
window.geometry("1000x310+" + str(randint(10, 900)) +
                "+" + str(randint(10, 500)))
window.wait_visibility(window)
window.wm_attributes("-transparentcolor", "gray99")

# Configuring grid
window.grid_columnconfigure(0, weight=1)
window.grid_columnconfigure(1, weight=1)
window.grid_columnconfigure(2, weight=100000)
window.grid_columnconfigure(3, weight=1000)
window.grid_columnconfigure(4, weight=0)
window.grid_columnconfigure(5, weight=0)
window.grid_columnconfigure(6, weight=0)

# Contents of widget
dummyFrame = tkinter.Frame(
    # Make sure that any transparent stuff has the bg `grey99` in it
    window,
    relief="flat",
    bg="grey99",
    height=200,
    width=297)
dummyFrame.pack()

dummy = tkinter.Label(
    dummyFrame,
    font="Segoe_UI 50",
    padx=5,
    text="Lorem Ipsum",
    pady=10,
    bg="grey99",
    bd=0,
    fg="white",
    relief="flat",
    height=10,
    width=36)
dummy.pack()

window.after(10, topOrNot())

window.bind("<Button-1>", getPos)

window.mainloop()
